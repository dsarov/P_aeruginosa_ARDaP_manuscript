...
while read SensitiveStrain; do
    if [ ! -s ${SensitiveStrain}_final_output/pheno_table.txt ]; then
    ...
    #  echo -e "Testing sensitive strain $SensitiveStrain\n"
	  ...
	  grep -w ${Antibiotic} ${SensitiveStrain}_final_output/pheno_table.txt > tmp.txt
...
if [ -s "$Antibiotic".R.txt ]; then
  while read ResistantStrain; do
    if [ ! -s ${ResistantStrain}_final_output/pheno_table.txt  ]; then
    ...
    # echo -e "Testing resistant strain $ResistantStrain\n"
	  ...
	  grep -w ${Antibiotic} ${ResistantStrain}_final_output/pheno_table.txt > tmp.txt
      awk -F "\t" '$3 ~ "Resistant" { exit 1} ' tmp.txt &> /dev/null
	  status=$? 
