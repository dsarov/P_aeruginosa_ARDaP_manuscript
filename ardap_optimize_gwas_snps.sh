...
cat snp.scores.full.$Ab | while read line; do 
  echo "$line" > tmp.var
  ...
  for i in barrio-tofino belkum buhl cabot cdc khaledi kos ramanathan sherrard tsang wardell; do
    for strain in ./"$i"/Outputs/AbR_reports/*.AbR_output.final.txt; do
      ...
      #echo $strain
      grep "$gene" ./$i/Outputs/Variants/Annotated/${strain_clean/AbR_output.final.txt/PASS.snps.annotated.vcf} | grep "$var" &> /dev/null         
      status=$? 
     if [ "$status" == 0 ]; then
