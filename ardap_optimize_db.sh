...
echo "APV including intermediates is $APV_current_int" >> APV.${Ab}.log
...
while read variant; do
   ...
   for i in barrio-tofino belkum buhl cabot cdc khaledi kos ramanathan sherrard tsang wardell; do
...
#
#
#  Intermediate variants are scored +2 if found in an intermediate strain
#                            scored +1 if found in a resistant strain
#                            scored -4 if found in a sensitive strain
#
...
#          scores resistance markers -1 if found in resistant isolates
#          scores resistance markers -4 if found in sensitive isolates
#
#
...
grep "${Ab}r" ${Ab}_database_score.txt | tail -n10 | awk -F" " ' $1 < 0 ' |  awk -F" " '{print $4"|"$5"|"$6"|" }' | while read line; do 
  if [ ! -z "$line" ]; then 
...
false_pos_false_neg=$(echo "$false_pos + $false_neg + $false_pos_c_int" | bc)
total=$(echo "$true_pos_true_neg + $false_pos_false_neg" | bc)
APV_current=$(echo "$true_pos_true_neg_true_int / $total_int" | bc -l)
...
if [[ "$APV_current" -eq "$APV_previous" ]]; then
  echo -e "No change to APV. Needs further investigation\n"
else 
  if [[ "$APV_previous" -lt "$APV_current" ]]; then
...
if [[ "$APV_current_int" -eq "$APV_previous_int" ]]; then
  echo -e "No change to APV. Needs further investigation\n"
else 
  if [[ "$APV_previous_int" -lt "$APV_current_int" ]]; then
    echo -e "APV has improved - ACCEPT\n"
  else
    echo -e "APV hasn't improved - REJECT\n"
  fi
